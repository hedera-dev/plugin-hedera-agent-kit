...
class OpenConvaiClient extends EventEmitter {
    ...
    private async prepopulateProcessedMessages(): Promise<void> {
        ...
        const activeConnections =
            this.connectionsManager.getActiveConnections();
        logger.info(
        ...
        for (const conn of activeConnections) {
            ...
            try {
                ...
                let lastTimestampForTopic =
                    this.lastProcessedTimestamps.get(topicId) ||
                    Date.now() - ONE_DAY_MS;
                ...
                for (const msg of history.messages) {
                    if (
                        ...
                        if (typeof msg.data === "string") {
                            ...
                            if (match && match[1]) {
                                const originalSequenceNumber = parseInt(
                                    match[1],
                                    10
                                );
    ...
    private async watchConnectionRequests(): Promise<void> {
        ...
        this._initializeTopicTracking(this.inboundTopicId);
        const inboundProcessed = this.processedMessages.get(
            this.inboundTopicId
        )!;
        ...
        try {
            ...
            for (const message of messages) {
                if (!message.sequence_number || message.sequence_number <= 0)
                    continue;
                ...
                if (!inboundProcessed.has(message.sequence_number)) {
                    ...
                    if (message.op === "connection_request") {
                        ...
                        const requesterAccountId =
                            this.client.extractAccountFromOperatorId(
                                message.operator_id
                            );
                        ...
                        if (newTopicId) {
                            ...
                            logger.info(
                                `Now monitoring new connection topic: ${newTopicId}`
                            );
                        }
        ...
        for (const conn of activeConnections) {
            ...
            try {
                const messagesResponse =
                    await this.client.getMessageStream(topicId);
                const messages = messagesResponse.messages;

                this.connectionsManager.processConnectionMessages(
                    topicId,
                    messages
                );
    ...
    private async finalizeConnection(
        ...
        try {
            ...
            if (sentReceipt?.topicSequenceNumber) {
                const processedSet =
                    this.processedMessages.get(connectionTopicId);
                if (processedSet) {
                    processedSet.add(
                        sentReceipt.topicSequenceNumber.toNumber()
                    );
                    logger.debug(
    ...
    private async handleMessage(
        ...
        if (messageContent.startsWith("hcs://")) {
            try {
                const content =
                    await this.client.getMessageContent(messageContent);
                messageContent =
        ...
        try {
            ...
            const userName =
                connectionInfo.targetAgentName ||
    ...
    private async _generateResponse(context: string): Promise<Content> {
        try {
            logger.debug(
                "Context passed to generateMessageResponse:",
                context.substring(0, 200) + "..."
            );
...
export const OpenConvaiClientInterface: ElizaClient = {
